...
| Approval modes | **PARTIAL** | `default`, `acceptEdits`, `plan`, `auto`, `dontAsk`, and `bypassPermissions` exist. `auto` now runs a versioned, redacted deterministic safety review after policy and before approval conversion; its exact-input decisions are bounded by expiry and recorded on the attempt. `bypassPermissions` skips that review but cannot cross hard policy denial. A bounded model reviewer for residual ambiguity is still open. See `crates/arsy-cli/src/approval.rs`, `turn.rs`, and `arsy-kernel/src/safety.rs`. |
...
| `task.spawn` | **PARTIAL** | Children run concurrently on bounded worker threads with durable attempts, reserved/settled budgets, attenuated grants, role contracts, result contracts, redacted observation, and immutable snapshots or isolated writer worktrees. The parent can status/wait/result/send/cancel/integrate without blocking on spawn. Recovery still reconstructs rather than resumes an in-flight provider stream. |
...
| Workspace isolation | **PARTIAL** | `WorkspaceRequirement::{ReadOnlySnapshot, IsolatedWriter}` and `WorkspaceCoordinator` are wired into child execution. Assignments, writer results, changed files, validation, lease epoch, and release state are durable; directories are process-owned and recovered from the event record. |
| Agent messaging and supervision | **PARTIAL** | Durable mailboxes, result/wait APIs, and a replayable Agent Hub projection exist. `/agents` and `session show` expose the projection; `/agents pause|resume|cancel|steer ATTEMPT` and typed service controls persist actor, causal predecessor, exact attempt, and result. Workers honor pause/cancel at model/tool boundaries. Policy-backed integrate and the four-agent accessibility/race gate remain open. |
...
|---|---|---|
| Claude Code | Official documentation describes foreground/background subagents, permission surfacing, worktree isolation, steering, teams, hooks, plugins, and task inspection. Its proprietary core cannot be source-verified. | ARSY has inspectable typed authority, durable concurrent child attempts, isolated writers, and cooperative supervision; recovery of an in-flight provider stream remains open. |
| OpenAI Codex / Codex CLI | Official documentation describes parallel subagents with inspectable threads and role/model configuration; current releases expose managed worktree and safety-review workflows. | ARSY has provider-neutral durable attempts, policy-backed role admission, deterministic Safe Auto review, and worktree isolation; residual model review and held-out role-routing evidence remain open. |
